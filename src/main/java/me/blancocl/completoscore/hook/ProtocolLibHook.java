...
public final class ProtocolLibHook {
	...
	public static void register() {
		...
		manager.addPacketListener(new PacketAdapter(CompletosCore.getInstance(), PacketType.Play.Client.CHAT) {
...
/*
		manager.addPacketListener(new PacketAdapter(CompletosCore.getInstance(), PacketType.Play.Server.PLAYER_INFO) {
			...
			public void onPacketSending(PacketEvent event) {
				...
				if (packet.getPlayerInfoAction().read(0) == EnumWrappers.PlayerInfoAction.ADD_PLAYER) {
					...
					for (int i = 0; i < list.size(); i++) {
						...
						if (CompletosCore.getPlayerTags().containsKey(uniqueId)) {
							String tag = CompletosCore.getPlayerTags().get(uniqueId);
